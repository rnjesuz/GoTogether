...
def cluster_route(request):
    ...
    for participant in event_participants:
        ...
        participant_distance_matrix[p.id] = {}
        participant_distance_matrix[p.id]["destination"] = direction_results[0].get(u'legs')[0].get(u'distance').get(u'value')

    number_participants = len(participants)
    waypoints = list(participants.keys())
    number_waypoints = number_participants + 1
    for participant in participants:
        # Populated with 0's
        participant_distance_matrix[participant] = {**dict(zip(waypoints, [0] * number_waypoints)), **participant_distance_matrix[participant]}
    ...
    cluster_distance = group_cells_distance(cluster, drivers_distance, participant_distance_matrix)
...
######################
def group_cells_distance(cluster_distance, drivers_distance, participants_distance_matrix):
    ...
    return group_best_match_drivers(cluster_distance, driver_to_driver_route_share, participants_distance_matrix)


def group_best_match_drivers(cluster_distance, driver_to_driver_route_share, participants_distance_matrix):
    global participants
    ...
    for donor in driver_seats:
        ...
        print("Routes shared: {}".format(driver_to_driver_route_share[donor]))
        for driver in reversed(driver_to_driver_route_share[donor]):
            ...
            if seats >= occupied_seats + (1 + len(cluster_distance.get(donor))):
                ...
                if verify_cumulative_distance(cluster_distance, driver, donor, participants_distance_matrix):
                    # join cars
